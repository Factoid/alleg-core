...
class   CasteroidIGC : public TmodelIGC<IasteroidIGC>
{
...
#else
        for( auto& pside : *(GetMission()->GetSides()) )
        {
#endif
...
#else
            oreSeenBySide[pside] = m_asteroidDef.ore;
#endif
					}
				}
			}
    ...
    // IasteroidIGC
		...
		{			
			...
			return oreSeen;
#else
      return oreSeenBySide.find(side1)->second;
#endif
		}
...
#else
      for( auto psl : *(GetMission()->GetSides()) )
      {
        if( !m_dead[psl->GetObjectID()] ) return false;
      }
#endif
			return true;
		}
