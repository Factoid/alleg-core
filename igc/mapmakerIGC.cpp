...
#else
    for( auto warp : *(p1->GetWarps()) )
    {
      IclusterIGC* p = warp->GetDestination()->GetCluster();
#endif
...
#else
    for( int i = pclusters->size() - 1; (i >= 0); --i )
#endif
    {
...
#else
    std::vector<IclusterIGC*> pclustersAdjacent(pclusters->size()*pclusters->size(),nullptr);
#endif
    int         nStart = 0;
    int         nStop = 0;
#ifdef WIN
...
#else
    for( auto pw : *(pcluster1->GetWarps()) )
    {
#endif
        if (pw != pwarp)
        {
...
#else
    if(!FindPath(pcluster2, &nStart, &nStop, &pclustersAdjacent[0], pclusterBackTrack, bVisited))
#endif
    {
        return false;
    }
