...
public:
  void Set( Time timeBase, Time timeNow )
  {
    timeOffset = (short)(timeBase-timeNow).count();
  }
...
#else
        Time  GetStateTime(void) const { return m_dTime; }
        void SetStateTime(const Time& t) { m_dTime = t; }
#endif
...
#else
        virtual TRef<ThingSite>      CreateThingSite(ImodelIGC* pModel) { return TRef<ThingSite>(new ThingSite); }
        virtual TRef<ClusterSite>    CreateClusterSite(IclusterIGC* pCluster){return TRef<ClusterSite>(new ClusterSite); }
#endif
...
#else
const ms c_dwFlashInterval(250);
const ms c_dwDamageSlotInterval(5000);
#endif
...
class   DamageTrack
{
    public:
        ...
        void    DeleteDamageBucket(DamageBucket*   pdb)
        {
            ...
            delete pdbl;
#else
            for( DamageBucketList::iterator i = m_buckets.begin(); i != m_buckets.end(); ++i )
            {
              if( (*i) == pdb )
              {
                m_buckets.erase(i);
...
class   DamageBucket
{
    public:
        ...
        bool flash(Time timeNow)
        {
          return (timeNow < m_timeLastDamage + c_dwFlashInterval) && (timeNow >= m_timeLastDamage);
        }
#endif
...
#else
            if( timeNow >= m_timeLastDamage + c_dwFlashInterval )
            {
              Time timeReset = m_timeLastDamage + 2*c_dwFlashInterval;
              m_timeLastDamage = (timeNow > timeReset) ? timeNow : timeReset;
            }
#endif
        }
...
class   DamageTrackSet
{
...
#else
            m_timeSlotChange = now + c_dwDamageSlotInterval;
#endif
        }
    ...
    private:
        ...
        void            DeleteTrack(DamageTrack*    pdt)
        {
            ...
            delete pdtl;
#else
            for( DamageTrackList::iterator i = m_tracks.begin(); i != m_tracks.end(); ++i )
            {
              if( (*i) == pdt )
              {
                m_tracks.erase(i);
...
#else
    for( BaseListIGC::iterator i = list->begin(); i != list->end(); ++i )
    {
      if( (*i) == base )
      {
        list->erase(i);
...
class PlayerScoreObject
{
    public:
        ...
        {
            return m_dtPlayed.count();
        }

        float GetTimeCommanded() const
        {
            return m_dtCommanded.count();
        }
