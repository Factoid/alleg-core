...
class       CclusterIGC : public IclusterIGC
{
    ...
    // IclusterIGC
		...
		virtual float                   GetHeliumSeen(IsideIGC* pside)
		{
			float fOre = 0.0;
#ifdef WIN
			for (AsteroidLinkIGC* asteriodLink = this->GetAsteroids()->first(); asteriodLink != NULL; asteriodLink = asteriodLink->next())
			{
				AsteroidAbilityBitMask aabm = asteriodLink->data()->GetCapabilities();
#else
      for( auto asteroid : *GetAsteroids() )
      {
        AsteroidAbilityBitMask aabm = asteroid->GetCapabilities();
#endif
...
#else
          fOre += asteroid->GetOreSeenBySide(pside);
#endif
				}
			}
