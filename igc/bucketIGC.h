...
class       CbucketIGC : public IbucketIGC
{
...
#else
                    ms dt = std::chrono::duration_cast<ms>(now - m_lastUpdate);
                    m_investedMilliseconds += dt.count();
#endif
    ...
    // IbuyableIGC
		...
		virtual bool					IsAvailableForFree() const
		{
...
#ifdef WIN
						for (ShipLinkIGC* shipLink = m_data.side->GetShips()->first(); shipLink != NULL; shipLink = shipLink->next())
						{
							if (shipLink->data()->GetBaseData() == pStationtype)
								return false;
						}
#else
            for( auto ship : *(m_data.side->GetShips()) )
            {
              if( ship->GetBaseData() == pStationtype ) return false;
            }
#endif
...
#ifdef WIN
						for (AsteroidLinkIGC* asteroidLink = m_pMission->GetAsteroids()->first(); asteroidLink != NULL; asteroidLink = asteroidLink->next()) 
						{
...
#else
            for( auto asteroid : *(m_pMission->GetAsteroids()) )
            {
              if( asteroid->HasCapability(aabm) && asteroid->SeenBySide(m_data.side) ) 
              {
                foundAsteroid = true;
                break;
              }
            ...
            for( auto ship : *(m_data.side->GetShips()) )
            {
              if( pt == ship->GetPilotType() ) ++droneCount;
            }
#endif
...
#else
                    IstationIGC* pstation;
                    StationListIGC::const_iterator i = m_data.side->GetStations()->begin();
                    while(true)
                    {
                      if( (*i)->CanBuy(m_data.buyable) )
                      {
                        pstation = (*i);
                        break;
                      }
                      ++i;
                      if( i == m_data.side->GetStations()->end() )
                      {
                        i = m_data.side->GetStations()->begin();
                      }
                    }
#endif
