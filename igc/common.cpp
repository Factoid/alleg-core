					   ...
					   int				   iAllies) //Imago 8/1/09
{
...
#else
                            for( auto ps : *(pside->GetMission()->GetShips()) )
                            {
#endif
...

    {
        ...
        do
        {
...
#else
                if(pwlTwoAway->empty() || (distance++ >= maxDistance))
#endif
                {
            ...
            if( ttMask & c_ttPrevious )
            {
              pwarp = pwlOneAway->back();
              pwlOneAway->pop_back();
            } else
            {
              pwarp = pwlOneAway->front();
...
#else
          for( auto ps : *(pmission->GetStations()) )
          {
#endif
...
#else
        for( auto pc : *(pmission->GetClusters()) )
        {
#endif
...
#else
    for( auto pcl : clustersRipcord )
    {
      ImodelIGC* pmodel = FindTarget(pship, ttMask, NULL, pcl.pcluster, pcl.pposition, NULL, abmAbilities, distance);
#endif
        if (pmodel)
        {
                   ...
                   ClusterQuality cq) //Spunky #288
{
...
#else
    explored.push_back(pclusterCurrent);
#endif

    {
        ...
        for( auto pwarp : *pwarps )
        {
            ...
            if (pship->CanSee(pwarp))
            {
                     ...
                     pclusterDestination->IsFriendlyCluster(pside, cq)) //Spunky #288
                {
                    ...
                    path.distance = (pwarp->GetPosition() - positionShip).LengthSquared();
                    path.pwarpStart = path.pwarp = pwarp;
                    unexplored.insert(std::find_if( unexplored.begin(), unexplored.end(), [&](decltype(unexplored.front()) p) { return path.distance < p.distance; } ), path);
#endif
                }
            }
        }
        ...
        if (unexplored.n() == 0)
            return NULL;
#else
        if( unexplored.empty() ) return NULL;
#endif
    }
...
#else
        if( unexplored.empty() ) return NULL;
        const Path& path = unexplored.front();
#endif
...
#else
        explored.push_back(pclusterNext);
#endif
...
#else
        for( auto pwarp : *(pclusterNext->GetWarps()) )
        {
#endif
            if (pship->CanSee(pwarp))
            {
...
#else
                    if( std::find(explored.begin(),explored.end(),pclusterDestination) == explored.end() )
#endif
                    {
#ifdef WIN
...
#else
                        Path path;
                        path.pwarpStart = path.pwarpStart;
                        path.pwarp = pwarp;
                        unexplored.push_back(path);
#endif
                    }
                }
            }
              ...
              float         tMax)
{
...
#ifdef WIN
    for (ModelLinkIGC*  pml = (bShipsOnly ? (ModelLinkIGC*)(pcluster->GetShips()->first()) : pcluster->GetPickableModels()->first()); (pml != NULL); pml = pml->next())
    {
...
#else
    auto lambda = [&](ImodelIGC* pmodel) { if ((pmodel != pship) &&
            (pmodel != pmodelIgnore) &&
            (pmodel->GetHitTest()->GetNoHit() != myHitTest))
    ...
    if( bShipsOnly )
    {
      for( auto pmodel : *(pcluster->GetShips()) )
      {
        lambda(pmodel);
      }
    } else
    {
      for( auto pmodel : *(pcluster->GetPickableModels()) )
      {
        lambda(pmodel);
      }
    }
                             ...
                             const ImodelIGC*   pmodel2)
{
    ...
    for (ModelLinkIGC* pml = ((ModelListIGC*)(pcluster->GetAsteroids()))->first(); (pml != NULL); pml = pml->next())
    {
        ImodelIGC*  pmodel = pml->data();
#else
    for( auto pmodel : *(pcluster->GetAsteroids()) )
    {
#endif
                        ...
                        float            bravery)
{
    ...
    if (pszName)
    {
        ...
        //Is the name unique?
        ShipLinkIGC*    psl;
        for (psl = pmission->GetShips()->first(); ((psl != NULL) && (_stricmp(pszName, psl->data()->GetName()) != 0)); psl = psl->next())
        {
        }
        ...
        auto same_name = [&] ( decltype(pmission->GetShips()->front()) pship ) { return std::string(pship->GetName()) == name; };
        if( std::any_of( pmission->GetShips()->begin(), pmission->GetShips()->end(), same_name ) )
        {
          _itoa(shipID, ds.name + strlen(ds.name), 10);
        }
#endif
...
#ifdef WIN
        for (PartTypeLinkIGC*   ptl = pht->GetPreferredPartTypes()->first(); (ptl != NULL); ptl = ptl->next())
        {
            IpartTypeIGC*   ppt = ptl->data();
#else
        for( auto ppt : *(pht->GetPreferredPartTypes()) )
        {
#endif
...
#else
    std::list<DamageBucket*> purge_list;
    for( auto db : m_buckets )
    {
      db->SwitchSlots(m_pset->m_idSlot);
      if( db->totalDamage() < 0.5f )
      {
        purge_list.push_back(db);
      }
    }
    for( auto db : purge_list )
    {
      delete db;
    }
#endif
                                 ...
                                 float       damage)
{
#ifdef WIN
    DamageBucketLink*  l;
    for (l = m_buckets.first(); ((l != NULL) && (l->data()->model() != pmodel)); l = l->next())
    {
    }
    ...
    if (l == NULL)
    {
        ...
        pBucket = l->data();
#else
    auto itr = std::find_if(m_buckets.begin(),m_buckets.end(),[&](decltype(m_buckets.front()) b) { return b->model() == pmodel; });
    DamageBucket* pBucket = nullptr; 
...
#else
    if( m_dtPlayed.count() == 0.0f ) return;
    float kMax = m_dtPlayed.count() / (15.0f*60.0f);
#endif
                ...
                if (pship->CanSee(w))
                {
                    ...
                    if (pwarpDestination)
                    {
                        ...
                        if (pclusterOther == pclusterStop)
                            ...
                            pwlTwoAway->last(pwarpDestination);
#else
                        else if( std::find(clustersVisited.begin(),clustersVisited.end(),pclusterOther) == clustersVisited.end() )
                        {
                          pwlTwoAway->push_back(pwarpDestination);
                        }
#endif
...
#else
        IstationTypeIGC* ppedestal = pmission->GetStationTypes()->back();
#endif
                            ...
                            short                   nAsteroidsMultiplier)
{
...
#else
        for( auto psl = pstations->rbegin(); psl != pstations->rend(); ++psl )
        {
            IstationIGC* pstation = *psl;
#endif
...

    {
        ...
        for( auto pstation : *pstations )
        {
            ...
            for( auto p : *psides )
            {
              if( p != pside ) pstation->SetSideVisibility(p,false);
            }
#endif
        }
    }
